  ...
  module OperationList
    ...
    def cleanup_input_table
      ...
      self.each do |op|
        ...
        # save true/false for boolean inputs
        # op.temporary[:boolean] = true
        # temp_op[:boolean_keys].each do |key|
        #   op.temporary[:boolean] = true
        # end
        # temp_op[:boolean_keys].each do |key|
        #   b = temp_op[:boolean_blocks][key]
        #   k, _ = key.to_s.split("||").map(&:to_sym)
        #   self.each do |op|
        #     v = op.temporary.delete(key)
        #     op.temporary[k] = b.call(v)
        #   end
        # end if temp_op[:boolean_keys]
    ...
    def show_with_input_table ops, create_block, num_tries=5
      ...
      while continue and counter < num_tries
        ...
        results = show do
          ...
          if input_table.choice.empty?
            table input_table.all
          else
